...
class adminService {
  ...
  async update(UserId, data) {
    ...
    if (data.Password) {
      throw new appError(
        404,
        "User not able to update password, if you want to update click forgot password"
      );
    }
    ...
    const Userqulification = await user_qualification.findOne({
      where: { User_info_id: UserId },
    });
    if (!Userqulification) {
      throw new appError(404, "User not found");
  ...
  async forgetPassword(Email, data) {
    ...
    const resetUrl = `${data.protocol}://${data.get(
      "host"
