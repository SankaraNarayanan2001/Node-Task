...
const validateRegistration = (req, res, next) => {
  ...
  const info = Joi.object({
    ...
    Nationality: Joi.string().trim().valid("Indian", "USA", "Dubai").messages({
    ...
    Occupation: Joi.string().allow(null).min(2).messages({
      "string.min": "Occupation should have a minimum length of {#limit}",
    }),
    Native_Place: Joi.string().trim().min(2).max(20).messages({
    ...
    First_Language: Joi.string().trim().min(2).max(20).messages({
    ...
    Degree: Joi.string().trim().allow("").optional().messages({
      "number.base": "Invalid year",
      "number.integer": "Year must be an integer",
      "number.positive": "Year must be a positive number",
    }),
    Institution: Joi.string().trim().allow("").optional().messages({
    ...
    FieldOfStudy: Joi.string().trim().allow("").optional().messages({
